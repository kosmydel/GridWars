 ...
 */
public class GluttonBotPRO6_T implements PlayerBot {
    ...
    public GluttonBotPRO6_T(int strategy_change,
            ...
            while (toMovePopulation > 0 && doneSomething) {
                ...
                for (int c = 0; c < okDirections.size(); c++) {
                    ...
                    if (movePopulation > 0) {
                        ...
                        nextState[p.getX()][p.getY()] -= movePopulation;
                        nextState[p.getNeighbour(dir).getX()][p.getNeighbour(dir).getY()] += movePopulation;
                        doneSomething = true;
