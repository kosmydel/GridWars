...
//    int[][] edgeDistance = new int[50][50];
    ...
    private static final int DENOMINATOR_VALUE = 1;
...
//            strategy2(universeView, commandList);
    ...
    public void strategy2(UniverseView universeView, List<MovementCommand> commandList) {
        ...
        for (Coordinates cell : myCells) {
            ...
            for(int j = 0; j < i; j++) {
                ...
                move(commandList, cell, directions.get(0), Math.min(currentPopulation / (i + DENOMINATOR_VALUE), 99 - neighbourPopulation));
                directions.remove(0);
            }
    ...
    public void generateGravityMap(UniverseView universeView) {
        ...
        List<Coordinates> borderNotEnemies = new ArrayList<>();
        ...
        while (!queue.isEmpty()) {
            ...
            int p_x = p.getX();
            int p_y = p.getY();
            // should be defined above gravity - 1
            if(gravity[p_x][p_y] == 0 && borderNotEnemies != null) {
                queue.addAll(borderNotEnemies);
                borderNotEnemies = null;
            }
            for (MovementCommand.Direction dir : MovementCommand.Direction.values()) {
